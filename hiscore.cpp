...
void hiscore::save_hiscores()
{
    ...
    if(scores.is_open())
    {
        for(int i = 0; i != 10; ++i)
        {
            ...
            scores.write(reinterpret_cast<char*>(&m_scores[i].score), sizeof(m_scores[i].score));
        }
    }
}
...
void hiscore::activate_state(const state_arg & args)
{
    ...
    if(const game_end_transition * t = boost::get<game_end_transition>(&args))
    {
        int score = 0;
        ...
        std::cout << "Play time: " << (t->time / 1000.0) << " seconds." << std::endl;
...
void hiscore::on_event(const SDL_Event & event)
{
    if(event.type == SDL_KEYDOWN && m_state == edit_name)
    {
        ...
        {
            if(m_current_score_pos != -1 && unsigned(m_current_score_pos) < m_scores.size())
            {
...
void hiscore::on_draw(sdl_surface & screen)
{
    ...
    for(unsigned i = 0; i != m_scores.size(); ++i)
    {
        ...
        os << (i+1) << ((i == unsigned(m_current_score_pos)) ? L": " : L". "); // TODO: Show the current line nicer
        m_font.render(screen,  80, 110 + i*24, os.str()); os.str(L"");
