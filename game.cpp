...
void game::do_special(special_move& special) {
  switch (special.m_type) {
    ...
    case special_move::shuffle: {
      ...
      for (int y = 0; y != 9; ++y)
        for (int x = 0; x != 9; ++x)
          if (m_board.get_digit(x, y) != -1) m_board.set_digit(x, y, *i++);
...
int game::clear_3x3(int xpos, int ypos) {
  int cleared = 0;
  int lx = std::max(0, xpos - 1);
  int hx = std::min(9, xpos + 2);
  int ly = std::max(0, ypos - 1);
  int hy = std::min(9, ypos + 2);
...
bool game::in_board() const { return m_selx != -1 && m_sely != -1; }
