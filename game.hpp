...
class tile_anim {
 public:
  tile_anim(sdl_surface& digits, int digit, int x, int y)
      : m_digits(digits), m_frame(0), m_digit(digit), m_x(x), m_y(y) {}

  bool tick() { return ++m_frame == 16; }

  void draw(sdl_surface& screen) {
    SDL_Rect src = {Sint16(m_digit * 32), 0, 32, Uint16(32 - m_frame * 2)};
    SDL_Rect dest = {Sint16(board_xpos + m_x * 32),
                     Sint16(board_ypos + m_y * 32 + m_frame * 2)};
...
class bonus_anim {
 public:
  bonus_anim(sdl_surface& srf, int cleared, int x, int y)
      : m_srf(srf), m_frame(0), m_cleared(cleared), m_x(x), m_y(y) {
    assert(m_cleared >= 4 && m_cleared <= 8);
  }

  bool tick() { return ++m_frame == 64; }

  void draw(sdl_surface& screen) {
    if (m_frame > 24 || m_frame & 1) {
      SDL_Rect src = {0, Sint16((m_cleared - 4) * 22), 95, 20};
      SDL_Rect dest = {Sint16(m_x), Sint16(m_y - (m_frame / 2))};
