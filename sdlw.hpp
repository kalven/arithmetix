...
class sdl_surface {
 public:
  ...
  sdl_surface(SDL_Surface *s) : m_surface(s) {
    if (s == 0) {
      throw std::logic_error("sdlw error: surface was null");
    }
  }

  sdl_surface(const sdl_surface &rhs) : m_surface(rhs.m_surface) {
    m_surface->refcount++;
  }
  ...
  void swap(sdl_surface &rhs) { std::swap(m_surface, rhs.m_surface); }
  ...
  int width() const { return m_surface ? m_surface->w : 0; }

  int height() const { return m_surface ? m_surface->h : 0; }

  bool null() const { return m_surface == 0; }
...
class sdl_surface_lock {
 public:
  sdl_surface_lock(sdl_surface &s) : m_surf(s) {
  ...
  Uint8 *data() {
    return reinterpret_cast<Uint8 *>(
        static_cast<SDL_Surface *>(m_surf)->pixels);
  }
