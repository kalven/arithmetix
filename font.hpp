...
class font
{
  public:
    ...
    void render(sdl_surface & screen, int x, int y, const std::wstring & text)
    {
        ...
        for(unsigned i = 0; i != text.size(); ++i)
        {
            ...
            if(ci != m_chars.end())
            {
                ...
                {
                    // Kerning
                    std::map<kern_pair, int>::iterator ki = m_kern.find(std::make_pair(text[i], text[i-1]));
                    if(ki != m_kern.end())
                    {
                        dest.x += ki->second;
                    }
                }
                SDL_Rect glyph_dest = { dest.x + c.xoffset, dest.y + c.yoffset }; 
    ...
    bool has_char(wchar_t c)
    {
        return m_chars.count(c) != 0;
    }
  ...
  private:
    ...
    typedef std::pair<wchar_t, wchar_t> kern_pair;
    struct char_info
    {
        SDL_Rect src;
        int xoffset, yoffset;
