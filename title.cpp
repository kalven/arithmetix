...
title::title(state_machine & sm) :
  ...
  m_beep(load_sound_resource("sounds/button.ogg")),
  m_sm(sm)
{
...
void title::on_event(const SDL_Event & event)
{
    ...
    if(event.type == SDL_MOUSEBUTTONDOWN)
    {
        ...
        {
            game_start_transition gs = { static_cast<unsigned int>(std::time(0)) }; // Starting seed
...
void title::draw_title(sdl_surface & screen)
{
    ...
    {
        dst.y = 100 + src[i].y;
        dst.y += static_cast<int>(std::sin((SDL_GetTicks() + (dst.x)) / 300.0) * 10.0);
        SDL_BlitSurface(m_title,
                        const_cast<SDL_Rect*>(&src[i]),
                        screen,
                        &dst);
        dst.x += xadd[i];
    }
