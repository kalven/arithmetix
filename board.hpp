...
class board {
 public:
  board() : m_digits(load_image_resource("digits.png")) { clear(); }
  ...
  void clear() { std::fill(m_board, m_board + 81, -1); }

  int get_3x3_sum(int x, int y) {
    int sum = 0;
    int lx = std::max(0, x - 1);
    int hx = std::min(9, x + 2);
    int ly = std::max(0, y - 1);
    int hy = std::min(9, y + 2);
  ...
  int get_digit(int x, int y) { return gb(x, y); }

  void set_digit(int x, int y, int digit) { gb(x, y) = digit; }

  int get_cleared() { return std::count(m_board, m_board + 81, -1); }

  int get_filled() { return 81 - get_cleared(); }
